...
/// SQL reserved words that collide in at least one supported backend
/// (SQLite / PostgreSQL / MySQL). Checked at content-type definition time so
/// `CREATE TABLE` / `INSERT … (col, …)` never hit a syntax error deep in a
/// migration. Conservative by design — better a rename hint than a 500.
...
/// Whether `name` is a SQL reserved word in any supported backend
/// (case-insensitive).
#[must_use]
pub fn is_reserved_keyword(name: &str) -> bool {
    let lower = name.to_ascii_lowercase();
    SQL_RESERVED.binary_search(&lower.as_str()).is_ok()
}
