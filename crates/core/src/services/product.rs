    ...
    async fn update(
        ...
        let updated = crate::models::product::update(
            ...
            &UpdateProductCmd {
                ...
                specs: req.specs.or(existing.specs.as_ref().map(|v| v.to_string())),
                unit: unit.to_string(),
                min_purchase,
    ...
    async fn batch(&self, auth: &AuthUser, action: &str, ids: &[String]) -> AppResult<usize> {
        ...
        for raw_id in ids {
            ...
            let cmd = UpdateProductCmd {
                ...
                status: status.to_string(),
                attributes: existing.attributes.as_ref().map(|v| v.to_string()),
                sort_order: existing.sort_order,
                slug: existing.slug,
                content: existing.content,
                image_ids: existing.image_ids,
                original_price: existing.original_price,
                specs: existing.specs.as_ref().map(|v| v.to_string()),
                unit: existing.unit,
