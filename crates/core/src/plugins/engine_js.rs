    ...
    pub async fn call_filter<T: Serialize + DeserializeOwned + Send>(
        ...
        let result: anyhow::Result<Option<T>> = ctx
            .with(|ctx| {
                ...
                let result_value: Value = match func.call((input_json,)) {
                    Ok(v) => v,
                    Err(e) => {
                        // Extract the actual JS exception message instead of
                        // the opaque "Exception generated by QuickJS" wrapper
                        // (same as call_action below).
                        ...
                        return Err(anyhow::anyhow!("JS error: {msg}"));
                    }
                };
                let result_str = ctx
    ...
    pub async fn call_action<T: Serialize>(
        ...
        let result: anyhow::Result<()> = ctx
            .with(|ctx| {
                ...
                match func.call::<(String,), ()>((data_json.clone(),)) {
                    Ok(()) => Ok(()),
                    Err(e) => {
                        // Extract the actual JS exception message instead of
                        // the opaque "Exception generated by QuickJS" wrapper.
