...
pub async fn list(
    ...
    let tags = post::list_tags(state.tag_repo.as_ref()).await?;
...
pub async fn create(
    ...
    let tag = post::create_tag(state.tag_repo.as_ref(), req).await?;
...
pub async fn delete(
    ...
    post::delete_tag(state.tag_repo.as_ref(), &id).await?;
