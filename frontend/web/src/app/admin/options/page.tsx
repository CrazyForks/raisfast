...
export default function OptionsPage() {
  ...
  const groupsQuery = useQuery({
    queryKey: ["options"],
    queryFn: () => client.send<OptionGroup[]>("/admin/options"),
  });
  ...
  const saveMutation = useMutation({
    mutationFn: (options: Record<string, unknown>) =>
      client.send<OptionGroup[]>("/admin/options", { method: "PUT", body: { options } }),
    onSuccess: (data) => {
    ...
    onError: (err) => {
      toast.error(err instanceof SDKError ? err.message : t("options.failedToSave"));
    },
  });
