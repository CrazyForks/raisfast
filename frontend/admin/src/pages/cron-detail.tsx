...
export default function CronDetailPage() {
  ...
  const scheduleQuery = useQuery({
    queryKey: ["cron", id],
    queryFn: () =>
      client.send<CronSchedule>(`/admin/crons/${encodeURIComponent(id)}`),
    retry: false,
  });
  ...
  const logsQuery = useQuery({
    ...
    queryFn: () =>
      client.send<CronExecutionLog[]>(
        `/admin/crons/logs`,
        { query: { schedule_id: encodeURIComponent(id), limit: "20" } },
      ),
  ...
  const cleanupMutation = useMutation({
    ...
    onError: (err) => {
      toast.error(
        err instanceof SDKError ? err.message : "Failed to cleanup",
      );
    },
  });
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="flex flex-col items-center gap-2 py-8 text-muted-foreground">
              ...
              <p>
                {scheduleQuery.error instanceof SDKError
