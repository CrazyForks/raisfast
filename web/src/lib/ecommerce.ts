...
async function pluginGet<T>(path: string, body?: object): Promise<T> {
  const res = body
    ? await apiRequest<PluginResponse<T>>(BASE + path, {
        method: "GET",
        body: JSON.stringify(body),
      })
    : await api.get<PluginResponse<T>>(BASE + path);
  if (!res.ok || !res.data) throw new Error(res.error ?? "Unknown error");
  return res.data;
}

async function pluginPost<T>(path: string, body?: object): Promise<T> {
  const res = await api.post<PluginResponse<T>>(BASE + path, body);
  if (!res.ok || !res.data) throw new Error(res.error ?? "Unknown error");
  return res.data;
}

async function pluginPut<T>(path: string, body?: object): Promise<T> {
  const res = await api.put<PluginResponse<T>>(BASE + path, body);
  if (!res.ok || !res.data) throw new Error(res.error ?? "Unknown error");
  return res.data;
}

async function pluginDelete<T>(path: string, body?: object): Promise<T> {
  const res = await api.delete<PluginResponse<T>>(BASE + path);
  if (!res.ok || !res.data) throw new Error(res.error ?? "Unknown error");
  return res.data;
}
